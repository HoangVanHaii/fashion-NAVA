...
const showToast = (message: string, success: boolean = true) => {
    isSuccess.value = success; // Set màu (Xanh/Đỏ)
    toastText.value = "";      // Reset text để trigger reactivity
    setTimeout(() => {
        toastText.value = message; // Set nội dung sau timeout để hiện lại
    }, 100);
};
...
// --- LOGIC MUA NGAY (GIỮ NGUYÊN NHƯ BẠN YÊU CẦU) ---
const handleBuyNow = () => {
    ...
    if (!token) {
        ...
        setTimeout(() => { router.push('/login'); }, 1500); // Delay chút để user đọc thông báo
        return;
    }
    ...
    cart.setCheckoutSession({
        ...
        voucher: null, // Giữ nguyên null như code gốc
...
const handleSubmitReply = async (parentId: string) => {
    ...
    if (success) {
        showToast("Đã gửi phản hồi thành công!", true); // True = Thành công (Xanh)
        replyingToId.value = null; 
        replyContent.value = "";
        replyFiles.value = []; 
    }
...
// --- [PHẦN LOGIC EDIT MỚI - ĐƯỢC CHÈN VÀO ĐÂY] ---
...
const handleDeleteChild = async (parentId: string, childId: string) => {
    ...
    // Vì store là void và tự alert, nhưng nếu muốn dùng toast đẹp thì gọi thêm
    showToast("Đã xóa thành công!", true); // True = Thành công
};

// --- CART LOGIC VỚI SHOW TOAST ---
const handleAddToCart = async (size?: IProductSizeResponse) => { 
    ...
    if (!login) {
        showToast("Vui lòng đăng nhập để thêm vào giỏ hàng!", false); // False = Lỗi (Đỏ)
        return;
    }
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 relative">
        <div class="lg:col-span-6 space-y-6">
          ...
          <div class="grid grid-cols-6">
            <div v-for="(imgSrc, i) in listImg" :key="i" class="aspect-square rounded-2xl overflow-hidden cursor-pointer border-2 transition-all duration-300 transform hover:-translate-y-1" :class="index === i ? 'border-black ring-2 ring-black/10' : 'border-transparent opacity-60 hover:opacity-100'" @click="index = i">
